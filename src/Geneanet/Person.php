...
class Person
{
    ...
    public function toHtml()
    {
        ...
        if (isset($this->half_siblings)) {
            ...
            foreach ($this->half_siblings as $rec) {
                $txt .= sprintf(
                    "<li><a href='?url=%s'>%s</a> + <a href='?url=%s'>%s</a></li>\n",
                    urlencode($rec['parents'][0]['url']),
                    $rec['parents'][0]['name'],
                    urlencode($rec['parents'][1]['url']),
                    $rec['parents'][1]['name']
                );
                $txt .= sprintf("<ul>\n");
    ...
    public function year($date, $lazy = false)
    {
        if ($date == null) {
            return '?';
        }
    ...
    public function ___getDate($date)
    {
        $date = trim($date);
        ...
        if (eregi("[0-9]{2}/[0-9]{2}/[0-9]{4}", $date)) {              // date with format 'dd/mm/aaaa' or 'mm/dd/aaaa'
            ...
            switch($this->fdate) {
                case "dd/mm/aaaa":
                    return ($tdate[0]*1)." ".$months[($tdate[1]*1)]." ".$tdate[2];
                break;
                case "mm/dd/aaaa":
                    return ($tdate[1]*1)." ".$months[($tdate[0]*1)]." ".$tdate[2];
                break;
            }
        }
